...
export default function AdminCategories(){
  ...
  async function save(){
    ...
    if(editing.id){
      ...
      if(error) return toast({ title: 'Error updating category', description: error.message, variant: 'destructive' });
      toast({ title: 'Category updated' });
    } else {
      const { error } = await supabase.from('categories').insert({ name: editing.name.trim() });
      if(error) return toast({ title: 'Error creating category', description: error.message, variant: 'destructive' });
  ...
  async function remove(id:string){
    ...
    if(error) return toast({ title: 'Error deleting category', description: error.message, variant: 'destructive' });
    toast({ title: 'Category deleted' }); await load();
  }
